...
import { OfertarVehículo } from '../../../../actions/Offer';

const VehicleOfferForm = ({ shipId,vehiclePosts, idOfertante }) => {
  const [post, setPost] = useState(vehiclePosts.length > 0 ? vehiclePosts[0].id : null);
  const [postError, setPostError] = useState(null);
  console.log(post)
  const router = useRouter();
  console.log(vehiclePosts.length)
  ...
  const onSubmit = async (data) => {
    ...
    console.log(post)
    console.log(shipId)
    const res = await OfertarVehículo({idOfertante: idOfertante, idPublicacionOfrecida: post, descripcion: data.descripcion,idPublicacionPedida: shipId});
    if (res.success) {
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      ...
      <CardHeader>
        ...
        <span className="text-slate-500 text-sm">Selecciona una publicación de un vehículo e ingresa una breve descripción para enviar tu oferta. </span>
      </CardHeader>
      <CardContent>
      <form onSubmit={onSubmitWithEvent}>
              ...
              <FormControl fullWidth>
                ...
                >
                  {vehiclePosts.length === 0 && (
                    <MenuItem value={null} className='text-sm'>No hay vehículos</MenuItem>
                  )}
                  {vehiclePosts.length > 0 && (
        ...
        { (postError === true || errors.descripcion) ? <span className="text-red-500 text-sm">Ingresa una descripción y selecciona una publicación de un vehículo</span>
        : (
